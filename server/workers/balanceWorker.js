...
import { createWorker, balanceQueue, QUEUE_NAMES } from '../config/queue.js';
...
// In-memory debounce fallback when Redis is unavailable
const inMemoryDebounce = new Map();
const DEBOUNCE_CLEANUP_INTERVAL = 60000; // Clean up expired entries every minute

// Periodic cleanup of expired in-memory debounce entries
setInterval(() => {
  const now = Date.now();
  for (const [key, expiry] of inMemoryDebounce.entries()) {
    if (expiry < now) {
 ...
 */
export const initBalanceWorker = async () => {
  const processBalanceJob = async (job) => {
    ...
    if (redisUp) {
      ...
      // Use in-memory debounce fallback
      const expiry = inMemoryDebounce.get(debounceKey);
      if (expiry && expiry > Date.now()) {
        // Debounced but no cache available without Redis, proceed with recalculation
        // (in-memory cache could be added here if needed for more aggressive debouncing)
      }
    }
    ...
    try {
      if (redisUp) {
        ...
        // Set in-memory debounce flag
        inMemoryDebounce.set(debounceKey, Date.now() + (DEBOUNCE_TTL * 1000));
      }
  ...
  // Create worker with concurrency 3 (uses BullMQ or MongoDB based on queue backend)
  const worker = await createWorker(QUEUE_NAMES.BALANCE, processBalanceJob, {
    concurrency: 3,
  });

  console.log('Balance worker initialized (concurrency: 3)');
  return worker;
};
...
export const invalidateBalanceCache = async (groupId) => {
  ...
  // Also clear in-memory debounce if it exists
  inMemoryDebounce.delete(`balance:debounce:${groupId}`);
};
